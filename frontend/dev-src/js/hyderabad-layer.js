 ...
 */
function addHyderabadLayers(map, apiBaseUrl = 'http://localhost:5000') {
  try {
    // Add vector tiles source for Hyderabad
    map.addSource('hyderabad-source', {
      type: 'vector',
      tiles: [`${apiBaseUrl}/collections/hyderabad/tiles/WebMercatorQuad/{z}/{y}/{x}?f=mvt`],
      minzoom: 0,
      maxzoom: 16,
      bounds: [78.237920, 17.290806, 78.621705, 17.561844]
    });
    ...
    map.on('click', 'hyderabad-layer', e => {
      ...
      for (const [key, value] of Object.entries(properties)) {
        if (!['ward_lgd_name', 'townname', 'state'].includes(key) && 
            key !== 'id' && key !== 'fid') {
